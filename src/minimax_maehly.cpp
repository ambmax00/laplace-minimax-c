...
float128 minimax::dEta(float128 x, Eigen::VectorXq& omegas, Eigen::VectorXq& alphas) 
{
...
float128 minimax::ddEta(float128 x, Eigen::VectorXq& omegas, Eigen::VectorXq& alphas) 
{
	...
	for (int ilap = 0; ilap != k; ++ilap) {
		...
		vals(ilap) = alphas(ilap) * alphas(ilap) * omegas(ilap) * 
			exp(-alphas(ilap) * x);
...
float128 minimax::newton(float128 x0, newton_function& update)
{
	...
	while (niter < _max_iter_newton) {
		...
		if (fabs(delta_x) < _itol_newton) break;
...
Eigen::VectorXq minimax::newton_maehly(double R, int k, 
	Eigen::VectorXq& omegas, Eigen::VectorXq& alphas)
{
	...
	for (int ixpt = 1; ixpt < 2*k; ++ixpt) {
		...
		newton_function nm_func = [this,&expts,&ixpt,&omegas,&alphas](float128 x) {
		  	auto de = dEta(x,omegas,alphas);
			auto dde = ddEta(x,omegas,alphas);

			float128 sum = 0.0;

			for (int jj = 0; jj < ixpt; ++jj) {
			  sum += pow(x - expts(jj), -1.0);
			}
