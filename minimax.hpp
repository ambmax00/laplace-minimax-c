...
private:
	...
	Eigen::VectorXq newton_maehly(double R, int k, Eigen::VectorXq& omega,
		Eigen::VectorXq& alpha);
		
	float128 newton(float128 x0, newton_function& update);
	
	float128 kahan_summation(Eigen::VectorXq& array);
	
	std::tuple<Eigen::VectorXq,Eigen::VectorXq> para_opt(Eigen::VectorXq& expts,
